// src/dbService/user.repository.ts
import {
  Injectable,
  ConflictException,
...
import { PostgresErrorCode } from '../db.enum';
...
export class UserRepository {
  ...
  async save(user: User): Promise<Partial<UserRow>> {
    ...
    try {
      const res = await this.dbService.query<Partial<UserRow>>(sql, values);
      return res.rows[0];
    } catch (error: unknown) {
  ...
  async logLogin(data: LoginLog): Promise<void> {
    const query = `
  ...
  async findByUsername(username: string): Promise<UserRow | null> {
    ...
    const sql = `SELECT * FROM users WHERE lower(username) = $1 LIMIT 1;`;
    const res = await this.dbService.query<UserRow>(sql, [normalized]);
